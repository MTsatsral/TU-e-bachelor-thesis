...
# Question: not sure whether the default values make sense
parser = argparse.ArgumentParser()
parser.add_argument('--data-dir', default='./', type=str, help='path to the data')
parser.add_argument('--data-tag', default='lab', type=str, help='files with data_tag in name under data directory will be used')
parser.add_argument('--num-splitted', default=10, type=int, help='number of files are splitted into')
...
def backward_loss(x, model, device): # device for cuda or cpu
    mu, logsigma, classcode = model.encoder(x)
    shuffled_classcode = classcode[torch.randperm(classcode.shape[0])]
